...
const AddTaskList = ({ AddNewTaskList }) => {
  ...
  const AddingTaskList = () => {
    return (
      <div className="flex flex-col h-20 bg-navbarBackground min-w-64 mr-4 flex-shrink-0 rounded-md mt-2">
        <div className="flex flex-col bg-taskCardBg rounded-lg m-1 h-10">
          <input
        ...
        <div className="flex items-center mx-1 rounded-md text-whiteText">
          <h2
            className="text-sm text-navbarBackground font-semibold rounded-sm py-1 px-2 p-3 mb-1 bg-menuButtonColor"
            onClick={() => {
              setAddingNewTaskList(false);
              AddNewTaskList(inputText);
              setInputText("");
  ...
  return addingNewTask
    ...
    : board.name && (
        <div
          className={`flex flex-row h-10 min-w-64 mt-2 mr-4 rounded-md flex-shrink-0 bg-addNewListBg items-center hover:bg-transparent`}
          onClick={() => {
            setAddingNewTaskList(true);
          }}
        >

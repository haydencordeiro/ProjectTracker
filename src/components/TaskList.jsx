...
const TaskList = forwardRef(({ taskListName, handleSort, AddCard }, ref) => {
  const tasks = useSelector((state) => state.tasks.value).filter(
    (task) => task.list === taskListName
  );
      ...
      <div className="flex-shrink-0 rounded-lg bg-taskListBg flex-column w-64 max-h-[559px] overflow-auto ">
        ...
        {tasks.map((task, index) => (
          ...
          >
            ...
            {task.dueDate ? (
              <div
                className={`text-xs p-1 w-16 justify-center items-center rounded-sm mb-1 ml-1 flex ${dueDateCalc(
                  task.dueDate
                )}`}
              >
        ...
        <div className="flex flex-col bg-taskCardBg rounded-lg m-1">
          <input
            className={`${
              addingNewTask ? "block p-2 pb-8" : "hidden"
            } bg-taskCardBg outline-none text-taskCardText`}
        ...
        {addingNewTask ? (
          <div className="bg-cyan-500 flex items-center hover:bg-red-500 mx-1 rounded-md text-whiteText">
            <h2
              className="text-sm text-navbarBackground font-semibold rounded-sm py-1 px-2 bg-menuButtonColor"
              onClick={() => {
                AddCard({ list: taskListName, task: inputText });
                setAddingNewTask(false);
              }}
            ...
            <MdClose
              className="rounded-sm mx-2 text-taskCardText text-lg z-10"
              onClick={() => {
                setInputText("");
                setAddingNewTask(false);
              }}
          ...
          <div
            className="bg-cyan-500 flex items-center hover:bg-red-500 mx-1 rounded-md text-whiteText hover:bg-backgroundNeutralHovered"
            onClick={() => {
              setInputText("");

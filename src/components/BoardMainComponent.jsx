...
import { updateTasks } from "../state/tasksSlice";
...
const BoardMainComponent = ({}) => {
    ...
    const fetchTasks = async () => {
      try {
        const response = await axios.get(
          `http://localhost:5000/board/api/getTasks/${board.boardId}`,
          {
  ...
  const swapListBackendApiHelper = async (first, second) => {
    const response = await axios.get(
      `http://localhost:5000/board/api/swaplist/${board.boardId}/${first}/${second}`,
      {
        withCredentials: true,
      }
    );
  ...
  const moveTaskApiHelper = async (first, second, newList) => {
    const response = await axios.get(
      `http://localhost:5000/board/api/movetask/${board.boardId}/${first}/${second}/${newList}`,
      {
        withCredentials: true,
      }
    );
  ...
  function handleSort(isEntireList = false) {
    ...
    if (isEntireList) {
      ...
      firstObjectIndex = allTaskListClone.findIndex(
        (obj) => obj === dragRef.current.dragTitleListName
      );
      secondObjectIndex = allTaskListClone.findIndex(
        (obj) => obj === dragRef.current.draggedOverTitleListName
      );
      ...
      firstObjectIndex = allTaskListClone.findIndex(
        (obj) => obj.id === dragRef.current.dragTask
      );
      secondObjectIndex = allTaskListClone.findIndex(
        (obj) => obj.id === dragRef.current.draggeOverTask
      );
    }
    ...
    if (!isEntireList) {
      ...
      moveTaskApiHelper(
        firstObjectIndex,
        secondObjectIndex,
        dragRef.current.draggedOverListName
      );
    }
    ...
    isEntireList
      ? setTaskList(allTaskListClone)
      : dispatch(updateTasks(allTaskListClone));
  }
  ...
  async function AddCard(taskData) {
    ...
    temp.push(taskData);
    const response = await axios.get(
      `http://localhost:5000/board/api/addTask/${board.boardId}/${taskData.task}/${taskData.list}`,
      {
        withCredentials: true,
      }
    );
  ...
  return (
    <div
      style={{ "--image-url": `url(${board.boardImageURL})` }}
    ...
    >
      ...
      <div className="flex overflow-auto">
        ...
        <AddTaskList AddNewTaskList={AddNewTaskList}></AddTaskList>
      </div>
    </div>
  );
};
